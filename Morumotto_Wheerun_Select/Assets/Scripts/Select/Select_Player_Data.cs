...
[System.Serializable]
public class Select_Player_Data : MonoBehaviour
{
    ...
    private Select_Player_Data player_data;

    public void Save_Data(Select_Player_Data player_data)
    {
    ...
    public void Load_Data(Select_Player_Data player_data, string datapath)
    {
        // ユーザごとに保管するディレクトリが異なる為、Pathを再度設定
        datapath = Application.dataPath + "/data/data.json";
        // パスを読み込む
    ...
    public void Init()
    {
        player = GetComponent<Select_Player>();
        player_data = GameObject.Find("Canvas").GetComponent<Select_Player_Data>();
        Load_Data(player_data, player_data.datapath);
        max_stage = 5;
        player_data.delete_start = false;
        if (player.sence == Select_Player.Character_Sence.NEXT_STAGESELECT)
        {
            Clear_Init();
        }
    }
    ...
    void Update()
    {
        if (player.sence == Select_Player.Character_Sence.GAME_DATADELETE)
        {
